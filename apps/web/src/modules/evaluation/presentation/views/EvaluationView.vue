...
const requiredScopes: ManagementScope[] = ['evaluation:manage'];
const canManageEvaluation = computed(() =>
  accessStore.hasScopes(requiredScopes),
);
...
async function initializeEvaluation(): Promise<void> {
  await evaluationStore.initialize();
...
async function loadAuthenticatedEvaluation(): Promise<void> {
  logout();
...
onMounted(async () => {
  const [, authenticated] = await Promise.all([
    ...
    accessStore.validateSession(),
  ]);

  if (authenticated && canManageEvaluation.value) {
    await loadAuthenticatedEvaluation();
  }
});
</script>
...
<template>
  <div class="admin-page evaluation-page">
    <section class="evaluation-hero">
      ...
      <div v-if="canManageEvaluation" class="evaluation-actions">
        <button
    ...
    <ManagementAccessPanel
      permission-message="当前管理凭证缺少评估管理权限。"
      :required-scopes="requiredScopes"
      @authenticated="loadAuthenticatedEvaluation"
      @logged-out="logout"
    />

    <template v-if="canManageEvaluation">
      <p v-if="evaluationStore.errorMessage" class="form-error" role="alert">
        ...
        <div class="evaluation-suite-list">
          ...
          >
            <header class="resource-card__header">
              ...
              <span
                class="status-badge"
                :class="`status-badge--${suite.status}`"
              >
