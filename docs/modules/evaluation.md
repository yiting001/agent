...
Observability generation feedback 和 Evaluation 分别解决线上信号与离线回归：

- 线上反馈关联真实聊天的 `generationId`，记录点赞、点踩、原因和评论，用于定位
  单次回答及聚合质量趋势。
- Evaluation 使用管理员维护的固定用例和确定性指标，支持重复运行与版本回归。
- 本期不会把负反馈自动写入评估集，避免未经审核的隐私内容、恶意输入和偶发噪声
  污染基准数据。
- 后续可增加人工审核队列，把确认后的脱敏输入、期望结果和 generation 配置快照
  转换为 Evaluation case。
