...
请求体可传入 `conversationId` 和服务端签发的 `memoryOwnerToken`。服务端校验 HMAC
签名并派生内部 ownerKey；同一 owner 下同一 ID 的成功对话会写入服务端短期记忆；用户显式要求
“记住”的事实或偏好会进入稳定长期记忆。owner 绑定图片会在回答结束后异步形成图片情景，后续可通过“上次那只狗”等指代召回。
...
- `X-Agent-Id` 与 `X-Memory-Owner-Token`：可选但必须同时提供；Vue 和 EyouCMS 使用它们绑定可形成长期情景的附件。
- 返回值：附件 ID、文件名、MIME 类型和大小。

对话消息通过附件 ID 引用文件。服务端重新读取并校验实际存储内容，然后转换为
OpenAI 兼容的 `image_url` 或 `input_audio` 内容片段。owner 绑定附件只能由相同 `agentId + ownerKey` 读取；旧无 owner 附件仅兼容无状态当前对话，不会被记录为长期情景。界面每条消息最多选择 6 个、
...
- `ChatAttachmentModule` 独立提供上传和存储能力，聊天与记忆模块复用同一端口，避免循环依赖。
...
- ECharts 使用 `echarts/core`、Canvas renderer、两组通用组件和按系列族拆分的
  ...
  `ResizeObserver` 和组件卸载时的 `dispose()` 生命周期不变。
- Mermaid 继续使用 `securityLevel: strict`，并把配置、渲染核心和各 diagram
