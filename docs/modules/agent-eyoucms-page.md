...
- 点击常用问题快速发起对话。
- 未配置智能体 ID 时，从平台已发布智能体中选择要对话的智能体；配置后只能使用指定的那一个。
- 清空当前对话。
- 在手机端打开或关闭对话列表。
...
`/api/public/agents/:agentId/chat`，只有 `published` 智能体可用；`draft` 与 `disabled`
都会被后端拒绝。后端根据智能体配置完成知识检索与真实模型调用。
...
6. 可选：创建 `agent_id` 自定义字段并填写已发布智能体 ID；
   不填写时访问者可在页面上自行选择平台已发布智能体；填写后只能使用该智能体。
