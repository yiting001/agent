...
- 智能体创建、编辑、删除、知识模块绑定、发布和停用。
- 大容量知识库、可复用模块和分片文档上传。
- API 应用及一次性访问密钥。
- 软件名称与图标的全局品牌配置。
- 基于真实模型和 Zvec 检索结果的对话测试。
...
- 编辑复用创建时的名称、描述、模型、提示词、温度和知识模块校验，保留智能体状态、创建时间和对话次数。
- 删除前由管理页面二次确认，后端确认智能体存在后删除聚合根。
- SQLite 外键负责级联删除智能体的模块绑定和 API 接入应用，不删除共享知识库、知识模块或文档。

```mermaid
sequenceDiagram
  participant View as 智能体管理页
  participant Store as AdminWorkspace Store
  participant API as NestJS Agents API
  participant UseCase as 编辑/删除用例
  participant DB as SQLite

  View->>Store: 保存修改或确认删除
  Store->>API: PUT/DELETE /agents/:id
  API->>UseCase: 执行业务规则
  UseCase->>DB: 更新配置或删除智能体
  DB-->>DB: 删除时级联清理绑定和接入应用
  DB-->>UseCase: 完成
  UseCase-->>Store: 智能体摘要或已删除 ID
  Store-->>View: 更新本地资源集合
```
...
- 管理端不存在业务演示数组或本地模拟回复。
- 页面创建、编辑、删除、配置、上传、发布和对话操作均调用 NestJS API。
- 5 个管理路由与独立对话路由保持中文。
- 单文件不超过 500 行。
