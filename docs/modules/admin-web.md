...
- 大容量知识库、可复用模块和分片文档上传。
- API 应用及一次性访问密钥。
- 内置系统提示词的查看、编辑、启停和并发冲突处理。
- 软件名称与图标的全局品牌配置。
- 基于真实模型和 pgvector 检索结果的对话测试。
- 对话测试自动携带 `conversationId` 和服务端签发的浏览器匿名
  `memoryOwnerToken`，附件上传同时绑定当前智能体和派生 owner；服务端可保存隔离的短期会话、长期偏好和图片情景。
...
| `/skills`          | 技能管理   | 安装提示词技能和 MCP 工具                  |
| `/prompt-policies` | 提示词管理 | 编辑和启停内置系统提示词                   |
...
- 提示词配置使用独立 `prompt-policies` 模块、Store 和 Gateway，通过 revision
  乐观锁防止多个管理员覆盖编辑。
...
测试页首次加载时向服务端签发匿名 `memoryOwnerToken`，后续提交当前消息记录、
`conversationId`、token 和智能体标识。后端验证签名并派生内部 ownerKey，再恢复该 owner 的会话最近消息，
...
- 浏览器只保存匿名 bearer token，不保存或接收内部 ownerKey；初始化失败时禁用输入、
  展示错误并允许重试，同时清理旧版客户端生成的 `agent-memory-owner`。
- 公开页面不得展示后台配置。
